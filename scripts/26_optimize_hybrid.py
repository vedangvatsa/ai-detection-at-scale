#!/usr/bin/env python3
"""Optimize BERT + stylometric combination per benchmark using logistic regression."""
import os
import sys
import numpy as np
import pandas as pd
...
def evaluate_benchmark(name, df, tokenizer, model, device, stylo_model, max_total=10000):
    print(f"\n=== {name} ===")
    ...
    for i, row in sample.iterrows():
        ...
        sem = get_semantic_prob(text, tokenizer, model, device)
        sty = get_stylometric_prob(text, stylo_model)
        records.append({'semantic': sem, 'stylometric': sty, 'label': label})

    data = pd.DataFrame(records)
    print(f"Evaluated {len(data)} samples")

    X = data[['semantic', 'stylometric']].values
    y = data['label'].values

    # Train a logistic regression on 50% of data, test on 50%
    ...
    sem_train_auc = roc_auc_score(y_train, X_train[:, 0])
    sem_test_auc = roc_auc_score(y_test, X_test[:, 0])
    ...
    print(f"Semantic: train AUC {sem_train_auc:.4f}, test AUC {sem_test_auc:.4f}")
...
def main():
    ...
    results_df.to_csv(os.path.join(PROJECT_DIR, 'results', 'hybrid_optimized_results.csv'), index=False)
    print("\nSaved to results/hybrid_optimized_results.csv")
